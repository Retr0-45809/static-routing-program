...
int main()
{
    ...
    for(i=0;i<k;i++)
    {
        do
        {
            printf("\nHow many networks are joint in router %d: ",i+1);
            scanf("%d",&num[i]);
            if(num[i]>=0)
                count+=num[i];
...
lable2:printf("\n");
    ...
    if(check1==0)
    {
        printf("\nDestination router is %d",router_no1);
    }
        dnc=router_no1;
        int check11=0;
        int final1=0;
    ...
    if(n[rnc-1][dnc-1]==1)
        {
            printf("\nDo you want to choose short path for routing purpose->1:");
            scanf("%d",&i4);
                if(i4==1){
                    printf("\nSource IP address:%s \n--> Source Router: %d \n-->Destination Router: %d \n-->Destination IP address:%s",source,router_no,router_no1,destination);
                    strcat(source,"*");
